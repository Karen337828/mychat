...
type Message struct {
	...
	TargetId   int64  //接受者用户Id
	Type       int    //发送类型  1私聊  2群聊  3心跳
	Media      int    //消息类型  1文字 2表情包 3图片
	Content    string //消息内容
...
func JoinGroup(userId uint, comId string) (int, string) {
	...
	clients.Mysql.Where("id=? or name=?", comId, comId).Find(&group)
	if group.Name == "" {
		return -1, "没有找到群"
	}
	clients.Mysql.Where("owner_id=? and target_id=? and type =2 ", userId, comId).Find(&contact)
	if !contact.CreatedAt.IsZero() {
		return -1, "已加过此群"
	} else {
		contact.TargetId = group.ID
		clients.Mysql.Create(&contact)
		return 0, "加群成功"
	}
}
...
func sendMsg(userId int64, msg []byte) {
	...
	r, err := clients.Redis.Get(ctx, "online_"+userIdStr).Result()
	if err != nil {
		fmt.Println(err)
	}
	if r != "" {
	...
	res, err := clients.Redis.ZRevRange(ctx, key, 0, -1).Result()
	if err != nil {
		fmt.Println(err)
	}
	score := float64(cap(res)) + 1
	ress, e := clients.Redis.ZAdd(ctx, key, &redis.Z{score, msg}).Result() //jsonMsg
	if e != nil {
		fmt.Println(e)
	}
	fmt.Println(ress)
...
func RedisMsg(userIdA int64, userIdB int64, start int64, end int64, isRev bool) []string {
	...
	var err error
	if isRev {
		rels, err = clients.Redis.ZRange(ctx, key, start, end).Result()
	} else {
		rels, err = clients.Redis.ZRevRange(ctx, key, start, end).Result()
	}
	if err != nil {
